...
@section('header')
  <section class="content-header">
    <h1>
      <span>{{ ucfirst($crud->entity_name) }}</span> {{ trans('lecuong::crud.revisions') }}
    </h1>
    <ol class="breadcrumb">
      <li><a href="{{ url(config('lecuong.base.route_prefix'),'dashboard') }}">{{ trans('lecuong::crud.admin') }}</a></li>
      <li><a href="{{ url($crud->route) }}" class="text-capitalize">{{ $crud->entity_name_plural }}</a></li>
      <li class="active">{{ trans('lecuong::crud.revisions') }}</li>
    </ol>
  </section>
@endsection
    ...
    @if(!count($revisions))
      <div class="box">
        <div class="box-header with-border">
          <h3 class="box-title">{{ trans('lecuong::crud.no_revisions') }}</h3>
        </div>
      </div>
    @else
...
@section('after_styles')
  ...
  <link rel="stylesheet" href="{{ asset('vendor/lecuong/crud/css/revisions.css') }}">
@endsection

@section('after_scripts')
  <script src="{{ asset('vendor/lecuong/crud/js/crud.js') }}"></script>
  <script src="{{ asset('vendor/lecuong/crud/js/revisions.js') }}"></script>
@endsection