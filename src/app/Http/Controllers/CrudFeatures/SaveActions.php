...
trait SaveActions
{
    ...
    {
        $saveAction = session('save_action', config('lecuong.crud.default_save_action', 'save_and_back'));
        $saveOptions = [];
        $saveCurrent = [
            'value' => $saveAction,
    ...
    public function setSaveAction($forceSaveAction = null)
    {
        if ($forceSaveAction) {
            ...
            $saveAction = \Request::input('save_action', config('lecuong.crud.default_save_action', 'save_and_back'));
        }

        if (session('save_action', 'save_and_back') !== $saveAction) {
            \Alert::info(trans('lecuong::crud.save_action_changed_notification'))->flash();
        }
    ...
    private function getSaveActionButtonName($actionValue = 'save_and_back')
    {
        switch ($actionValue) {
            case 'save_and_edit':
                return trans('lecuong::crud.save_action_save_and_edit');
                break;
            case 'save_and_new':
                return trans('lecuong::crud.save_action_save_and_new');
                break;
            case 'save_and_back':
            default:
                return trans('lecuong::crud.save_action_save_and_back');
                break;
        }
    }
}
