...
@section('header')
	<section class="content-header">
	  <h1>
	    ...
	    <small>{{ trans('lecuong::crud.all') }} <span>{{ $crud->entity_name_plural }}</span> {{ trans('lecuong::crud.in_the_database') }}.</small>
	  </h1>
	  <ol class="breadcrumb">
	    <li><a href="{{ url(config('lecuong.base.route_prefix'), 'dashboard') }}">{{ trans('lecuong::crud.admin') }}</a></li>
	    <li><a href="{{ url($crud->route) }}" class="text-capitalize">{{ $crud->entity_name_plural }}</a></li>
	    <li class="active">{{ trans('lecuong::crud.list') }}</li>
	  </ol>
	</section>
@endsection
            ...
            <thead>
              <tr>
                ...
                @if ( $crud->buttons->where('stack', 'line')->count() )
                  <th>{{ trans('lecuong::crud.actions') }}</th>
                @endif
              </tr>
            </thead>
            <tbody>
              ...
              @if (!$crud->ajaxTable())
                ...
                <tr data-entry-id="{{ $entry->getKey() }}">
                  ...
                  @foreach ($crud->columns as $column)
                    ...
                    @else
                      @if(view()->exists('vendor.lecuong.crud.columns.'.$column['type']))
                        @include('vendor.lecuong.crud.columns.'.$column['type'])
                      @else
...
@section('after_styles')
  ...
  <link rel="stylesheet" href="{{ asset('vendor/lecuong/crud/css/form.css') }}">
  <link rel="stylesheet" href="{{ asset('vendor/lecuong/crud/css/list.css') }}">
...
@section('after_scripts')
    ...
    <script src="{{ asset('vendor/lecuong/crud/js/form.js') }}"></script>
    <script src="{{ asset('vendor/lecuong/crud/js/list.js') }}"></script>
	...
	<script type="text/javascript">
      ...
      @endif
        ...
        "aaSorting": [],
        "language": {
              "emptyTable":     "{{ trans('lecuong::crud.emptyTable') }}",
              "info":           "{{ trans('lecuong::crud.info') }}",
              "infoEmpty":      "{{ trans('lecuong::crud.infoEmpty') }}",
              "infoFiltered":   "{{ trans('lecuong::crud.infoFiltered') }}",
              "infoPostFix":    "{{ trans('lecuong::crud.infoPostFix') }}",
              "thousands":      "{{ trans('lecuong::crud.thousands') }}",
              "lengthMenu":     "{{ trans('lecuong::crud.lengthMenu') }}",
              "loadingRecords": "{{ trans('lecuong::crud.loadingRecords') }}",
              "processing":     "{{ trans('lecuong::crud.processing') }}",
              "search":         "{{ trans('lecuong::crud.search') }}",
              "zeroRecords":    "{{ trans('lecuong::crud.zeroRecords') }}",
              "paginate": {
                  "first":      "{{ trans('lecuong::crud.paginate.first') }}",
                  "last":       "{{ trans('lecuong::crud.paginate.last') }}",
                  "next":       "{{ trans('lecuong::crud.paginate.next') }}",
                  "previous":   "{{ trans('lecuong::crud.paginate.previous') }}"
              },
              "aria": {
                  "sortAscending":  "{{ trans('lecuong::crud.aria.sortAscending') }}",
                  "sortDescending": "{{ trans('lecuong::crud.aria.sortDescending') }}"
              }
          },
      ...
      function register_delete_button_action() {
        ...
        $("[data-button-type=delete]").click(function(e) {
          ...
          if (confirm("{{ trans('lecuong::crud.delete_confirm') }}") == true) {
              $.ajax({
                  url: delete_url,
                  type: 'DELETE',
                  success: function(result) {
                      ...
                      new PNotify({
                          title: "{{ trans('lecuong::crud.delete_confirmation_title') }}",
                          text: "{{ trans('lecuong::crud.delete_confirmation_message') }}",
                          type: "success"
                      });
                  ...
                  error: function(result) {
                      // Show an alert with the result
                      new PNotify({
                          title: "{{ trans('lecuong::crud.delete_confirmation_not_title') }}",
                          text: "{{ trans('lecuong::crud.delete_confirmation_not_message') }}",
                          type: "warning"
                      });
                  }
              });
              ...
              new PNotify({
                  title: "{{ trans('lecuong::crud.delete_confirmation_not_deleted_title') }}",
                  text: "{{ trans('lecuong::crud.delete_confirmation_not_deleted_message') }}",
                  type: "info"
              });
          }
        });
        ...
        $('#crudTable tbody td.details-control').on('click', function () {
            ...
            if ( row.child.isShown() ) {
                ...
                .fail(function(data) {
                  ...
                  row.child("<div class='table_row_slider'>{{ trans('lecuong::crud.details_row_loading_error') }}</div>").show();
                  tr.addClass('shown');
                  $('div.table_row_slider', row.child()).slideDown();
                })
