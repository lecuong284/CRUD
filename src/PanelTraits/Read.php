...
namespace Lecuong\CRUD\PanelTraits;
...
trait Read
{
    ...
    */
    ...
    public function getDefaultPageLength()
    {
        ...
        if (config('lecuong.crud.default_page_length')) {
            return config('lecuong.crud.default_page_length');
        }
    ...
    public function getCellView($column, $entry)
    {
        if (! isset($column['type'])) {
            ...
            if (view()->exists('vendor.lecuong.crud.columns.'.$column['type'])) {
                return \View::make('vendor.lecuong.crud.columns.'.$column['type'])->with('crud', $this)->with('column', $column)->with('entry', $entry)->render();
            } else {
