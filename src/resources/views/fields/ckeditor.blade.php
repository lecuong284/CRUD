...
@if ($crud->checkIfFieldIsFirstOfItsType($field, $fields))
    ...
    @push('crud_fields_scripts')
        <script src="{{ asset('vendor/lecuong/ckeditor/ckeditor.js') }}"></script>
        <script src="{{ asset('vendor/lecuong/ckeditor/adapters/jquery.js') }}"></script>
    @endpush
...
<script>
    jQuery(document).ready(function($) {
        $('textarea[name="{{ $field['name'] }}"].ckeditor').ckeditor({
            "filebrowserBrowseUrl": "{{ url(config('lecuong.base.route_prefix').'/elfinder/ckeditor') }}",
