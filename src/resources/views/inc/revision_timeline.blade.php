...
@foreach($revisions as $revisionDate => $dateRevisions)
  <li class="time-label" data-date="{{ date('Y-m-d', strtotime($revisionDate)) }}">
      <span class="bg-red">
        {{ Date::parse($revisionDate)->format(config('lecuong.base.default_date_format')) }}
      </span>
  </li>
  ...
  <li class="timeline-item-wrap">
    ...
    <div class="timeline-item">
      ...
      @if($history->key == 'created_at' && !$history->old_value)
        <h3 class="timeline-header">{{ $history->userResponsible()?$history->userResponsible()->name:trans('lecuong::crud.guest_user') }} {{ trans('lecuong::crud.created_this') }} {{ $crud->entity_name }}</h3>
      @else
        <h3 class="timeline-header">{{ $history->userResponsible()?$history->userResponsible()->name:trans('lecuong::crud.guest_user') }} {{ trans('lecuong::crud.changed_the') }} {{ $history->fieldName() }}</h3>
        <div class="timeline-body p-b-0">
          <div class="row">
            <div class="col-md-6">{{ ucfirst(trans('lecuong::crud.from')) }}:</div>
            <div class="col-md-6">{{ ucfirst(trans('lecuong::crud.to')) }}:</div>
          </div>
          <div class="row">
        ...
        <div class="timeline-footer p-t-0">
          ...
          <button type="submit" class="btn btn-primary btn-sm restore-btn" data-entry-id="{{ $entry->id }}" data-revision-id="{{ $history->id }}" onclick="onRestoreClick(event)">
            <i class="fa fa-undo"></i> {{ trans('lecuong::crud.undo') }}</button>
          {!! Form::close() !!}
        </div>
      @endif
    </div>
    ...
    function onRestoreClick(e) {
      ...
      $.ajax('{{ \Request::url().'/' }}' +  revisionId + '/restore', {
        ...
        success: function(revisionTimeline) {
          ...
          new PNotify({
              text: '{{ trans('lecuong::crud.revision_restored') }}',
              type: 'success'
          });
        }
      });
